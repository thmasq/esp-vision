...
/// Vectorized single-precision float element-wise multiplication using ESP32-S3 SIMD extension.
#[inline(always)]
pub unsafe fn dsps_mul_f32_aes3_core(
    ptr_a: *const f32,
    ptr_b: *const f32,
    ptr_out: *mut f32,
    len: usize,
...
/// Hardware accelerated element-wise multiplication of two single-precision arrays.
pub fn esp_mul_f32(a: &[f32], b: &[f32], out: &mut [f32]) {
    ...
    unsafe {
        ...
        dsps_mul_f32_aes3_core(a.as_ptr(), b.as_ptr(), out.as_mut_ptr(), a.len())
    }
}

...
use crate::dsp::{AlignedDVec, AlignedDVecExt, EspDotProd, EspVectorMath};
use alloc::vec::Vec;
...
/// Fits a 2D line to a subset of boundary points using SIMD eigenvalue decomposition.
/// Returns: (MeanX, MeanY, NormalX, NormalY, MeanSquaredError)
pub fn fit_line_simd(points: &[Point]) -> Option<(f32, f32, f32, f32, f32)> {
    let n = points.len();
    if n < 2 {
        return None;
    }

    let mut xs_vec = AlignedDVec::<f32>::zeros(n);
    let mut ys_vec = AlignedDVec::<f32>::zeros(n);
    ...
    let ex = sum_x / (n as f32);
    let ey = sum_y / (n as f32);

    let mut ex_vec = AlignedDVec::<f32>::zeros(n);
    let mut ey_vec = AlignedDVec::<f32>::zeros(n);
    for i in 0..n {
        ex_vec[(i, 0)] = ex;
        ey_vec[(i, 0)] = ey;
    }

    // Shift all points so the centroid is at (0,0)
    let dx = xs_vec.esp_sub(&ex_vec);
    let dy = ys_vec.esp_sub(&ey_vec);

    // Compute Covariance Matrix elements
    let cxx = dx.esp_dot(&dx) / (n as f32);
    let cyy = dy.esp_dot(&dy) / (n as f32);
    let cxy = dx.esp_dot(&dy) / (n as f32);
...
pub fn find_quad_corners(boundary: &[Point]) -> Option<[Point; 4]> {
    ...
    // Cap it at 20, or sz/12 to handle highly skewed perspective tags.
    ...
    for i in 0..sz {
        ...
        if let Some((_, _, _, _, mse)) = fit_line_simd(&window_buf) {
            errs.push(mse);
        } else {
            errs.push(0.0);
        }
    ...
    for i in 0..4 {
        ...
        if let Some((ex, ey, nx, ny, _)) = fit_line_simd(&edge_points) {
            lines[i] = (ex, ey, nx, ny);
        } else {
            return None;
        }
